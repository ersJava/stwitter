package com.company.comment.viewmodel;
...
public class CommentViewModel {
    ...
    public CommentViewModel(@NotNull(message = "Post ID must have a value") int postId,
                            @Future(message = "Cannot be a past date")
                            @NotNull(message = "Please provide a date for the comment") LocalDate createDate,
                            @NotEmpty(message = "Please give a name for the comment")
                            @Size(min = 1, max = 50, message = "50 characters max for name") String commenterName,
                            @Size(max = 255, message = "Comment must be between 5 and 255 characters") String comment) {
        this.postId = postId;
